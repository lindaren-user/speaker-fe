<template>
    <el-card class="dataset-container-card">
      <template #header>
        ...
        <div>
          <ul>
            <li>视频数量限制为100，超出该数量的视频会被忽略</li>
            <li>视频尽可能覆盖所有使用场景，否则会导致实机部署效果欠佳</li>
          </ul>
  
          <ul>
            <li>每个标签的视频数量最少10份，否则训练效果可能一般</li>
            <li>每个标签的视频数量尽量接近</li>
          </ul>
        </div>
      </template>
      <div class="dataset-container">
          ...
          <!-- 摄像头录制对话框 -->
          <el-dialog v-model="showCameraDialog" title="摄像头录制">
  ...
  import request from '@/utils/formdata';
  ...
  const handleUploadAll = async () => {
    ...
    try {
      await Promise.all(unUploaded.map(uploadVideo));
  ...
  const uploadVideo = (video) => {
    ...
    return request.post('/api/upload', formData).then((response) => {
