<template>
    <div class="tagbody">
        ...
        <VideoTag :videoList="videoList" :isTagged="isTagged" :noTagged="noTagged" @video-selected="handleVideoSelected"></VideoTag>
        <el-dialog
        ...
        >
            ...
            <template #footer>
                <div class="dialog-footer">
                    <el-button v-if="changeObject.tag === true" type="danger" @click="deleteTag">删除标签</el-button>
...
const changeTag = () => {
    ...
    request.put("/api/updateTag", changeObject.value).then(res => {
        if (res.code === 200) {
            ElMessage({
                type: 'success',
                message: "标注成功"
...
const deleteVideo = (videoTitle) => {
    ElMessage.confirm('此操作将永久删除该视频, 是否继续?', '提示', {
        ...
        request.delete("/api/deleteVideo", {
            params: {
...
const deleteTag = () => {
    ...
    request.put("/api/deleteTag", null, {
        params: {
            videoTitle: videoTitle
        }
    }).then(res => {
